...
class GeminiEngine:
    ...
    def compose_dish_in_environment(self, dish_img_path, env_img_path):
        try:
            ...
            # Sécurité Pro : Ouverture propre du fichier avec fermeture automatique
            with open(dish_img_path, 'rb') as f:
                response = requests.post(
                    ...
                    timeout=15 # Sécurité Pro : Évite que le serveur attende indéfiniment si l'API bug
                )
            
            # Gestion Pro du quota ou des erreurs de clé
            if response.status_code != 200:
                print(f"[VISUAL ENGINE ERROR] Erreur API (Code {response.status_code}) : {response.text}")
                print("[VISUAL ENGINE PRO] Bascule sur le mode de secours (incrustation sans détourage)...")
            ...
            with Image.open(env_img_path).convert("RGBA") as env:
                ...
                # Effet de perspective : inclinaison à plat de l'assiette
                ...
                # Génération de l'ombre portée réaliste
                ...
                # Calcul du centrage sur la table
    ...
    def _fallback_composition(self, dish_img_path, env_img_path):
        """ Mode de secours Pro : si l'API est en panne, on évite le crash blanc """
        try:
            with Image.open(env_img_path).convert("RGB") as env:
                with Image.open(dish_img_path).convert("RGB") as dish:
                    dish.thumbnail((300, 300))
                    env.paste(dish, (20, 20)) # On colle temporairement le bloc dans un coin
                    output_path = dish_img_path.parent / "composed.jpg"
